...
// --- Commit #6: Boost flash popup state ---
let boostFlashFrames = 0;     // counts down frames for the popup
let boostFlashX = 0, boostFlashY = 0;
...
function applySpeedBoostIfNeeded() {
  if (boostToggle.checked && hitCount > 0 && hitCount % HITS_PER_BOOST === 0) {
    ...
    // start the "+10%" flash near the ball
    boostFlashFrames = 30; // ~0.5s at 60fps
    boostFlashX = ballX;
    boostFlashY = ballY;
  }
}
...
function draw() {
  ...
  // Commit #6: draw the "+10%" boost flash if active
  if (boostFlashFrames > 0) {
    const alpha = boostFlashFrames / 30; // fade out
    ctx.save();
